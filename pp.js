...
var sumB = new Array();
var ppMode = "paint";
var ppBoderLeft = parseInt($('#myCanvas').css('border-left-width'));
var ppBoderTop = parseInt($('#myCanvas').css('border-top-width'));
...
function ppDoDrag()
{
	var offsetX = 0, offsetY = 0, tempOffsetY = 0, tempOffsetY = 0;
	function ppDrag(curX, curY)
	{
		var dX = tempOffsetX = curX - ppInitialPoint.x;
		var dY = tempOffsetY = curY - ppInitialPoint.y;
		//console.info(dX+" "+dY);
		context.putImageData(ppImgData, dX + offsetX, dY + offsetY);
	}
...
function ppSetRGB(color)
{
	ppR = parseInt(color.substr(1, 2), 16);
	ppG = parseInt(color.substr(3, 2), 16);
	ppB = parseInt(color.substr(5, 2), 16);
}
...
function ppDoPick()
{
	...
	$('#myCanvas').mousemove(function(e)
	{
		if(pick)
		{
			var mouseX = e.pageX - this.offsetLeft - ppBoderLeft;
			var mouseY = e.pageY - this.offsetTop - ppBoderTop;
			var R = ppR = ppImgData.data[(mouseY * ppCanvasWidth + mouseX) * 4];
			var G = ppG = ppImgData.data[(mouseY * ppCanvasWidth + mouseX) * 4 + 1];
			var B = ppB = ppImgData.data[(mouseY * ppCanvasWidth + mouseX) * 4 + 2];
...
function ppDoPaint()
{
	...
	$('#myCanvas').mousedown(function(e)
	{
		if(e.which == 1)
		{
			context.strokeStyle = rgb(ppR, ppG, ppB);
			ppMousePressed = true;
			var mouseX = e.pageX - this.offsetLeft - ppBoderLeft;
			var mouseY = e.pageY - this.offsetTop - ppBoderTop;
...
function ppDoZoomIn()
{
	...
	$('#myCanvas').click(function(e)
	{
		var imageData = context.getImageData(0, 0, ppCanvasWidth, ppCanvasHeight);
		var newCanvas = $("<canvas>").attr("width", ppCanvasWidth).attr("height", ppCanvasHeight)[0];
...
function ppDoFill()
{
	...
	$('#range').change(function()
	{
		threshold = parseInt($('#range').val()) * 20;
	});
...
function ppDoT()
{
	...
	$('#myCanvas').click(function(e)
	{
		...
		strTemp += mouseX + ','+mouseY+')" style="font-family:Arial;background:transparent;position:absolute;color:white;left:';
		strTemp += x+'px;top:'+y+'px;'+'font-size:'+ ppFontSize + ';"></textarea>';
		$('#stage').append(strTemp);
	});
}
function ppPutStrOnCanvas(X, Y)
{
	context.font = ppFontSize*3 + "px Arial";
	context.fillStyle = "white";
	context.fillText($('#apd').val(), X, Y);
	console.info("success");
}

function ppSetModeFill()
{
	ppCommom();
	ppMode = "fill";
	ppDoFill();
...
function ppSetModePick()
{
	ppCommom();
	ppMode = "pick";
	ppDoPick();
}
function ppSetModeZoomIn()
{
	ppCommom();
	ppMode = "zoomin";
	ppDoZoomIn();
